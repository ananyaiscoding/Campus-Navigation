    ...
    const destMarker = L.marker([destination.lat, destination.lng]).addTo(map)
        ...
        function showRoute(userLat, userLng, dest) {
            ...
            routingControl = L.Routing.control({
                ...
                const distanceInKm = (best.summary.totalDistance / 1000).toFixed(2);
                const timeInMinutes = Math.round(best.summary.totalTime / 60);
                updateSidebarContent(distanceInKm, timeInMinutes);
            }).addTo(map);
        }
        ...
        function updateSidebarContent(distance, duration) {
            sidebar.style.display = "block";
            destName.innerText = destination.name;
            destImg.src = destination.image;
            destDistance.innerText = `${distance} km`;
            destTime.innerText = `${duration} min`;
            destTimings.innerText = destination.timings;
            destDesc.innerText = destination.description;
        }
        
        const constantUserPosition = {
            coords: {
                latitude: 26.4029485,
                longitude: 75.8756551
            }
        };
        updateUserPosition(constantUserPosition);
